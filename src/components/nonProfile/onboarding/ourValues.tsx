...
const OurValues = () => {
  ...
  const handleSaveAndContinue = async () => {
    try {
      ...
      navigate('/non-profit/create-account/skills');
    } catch (err: any) {
      console.error('❌ Failed to save values:', err);
      setError(err.response?.data?.message || 'Failed to save values. Please try again.');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="bg-white border border-black/30 my-5 rounded-md relative min-h-screen">
          ...
          <div className="my-10 px-5">
            ...
            <div className="border-t border-black/30 py-4 px-5 absolute bottom-0 right-0 w-full flex items-center justify-between">
              <div>
                <button 
                  onClick={() => navigate('/non-profit/create-account/mission-statement')}
